...
# copy logs file from remote server.
copy_logs()
...
# Verificar si la ruta a 7-Zip es correcta
if not os.path.isfile(seven_zip_path):
    raise FileNotFoundError(f"No se encontró 7-Zip en la ruta especificada: {seven_zip_path}")
...
def has_extension(file_name, extensions):
    """
    Verifica si el archivo tiene una de las extensiones especificadas.
    
    :param file_name: Nombre del archivo
    :param extensions: Lista de extensiones (por ejemplo, ['.zip', '.7z'])
    :return: True si el archivo tiene una de las extensiones, False en caso contrario
    """
...
for root, dirs, files in os.walk(logs_folder):
    for file_name in files:
        ...
        if has_extension(file_name, ['.zip', '.gz']):
            print("#*20","ARCHIVO A EXTRAER")
            extract_folder = root
            decompress_with_7zip(file_path, root)
            print("#####*10", "DECOMPRESS")
        
        if has_extension(file_name, ['.txt']) and file_name.startswith('Logs_status'):
            pass
                
        if has_extension(file_name, ['.log']):
